...
class BaseDataset(Dataset):
    ...
    def __init__(self, options, dataset, ignore_3d=False, use_augmentation=True, is_train=True, Time4D= False):
        super().__init__()
    ...
    def __getitem__(self, index):
        indexs = [np.max(0,index-1),index,np.min(len(self.imgname)-1 , index+1)] if self.Time4D else [index]
        item_list = []
        for index in indexs:
            item = {}
        ...
        return item_list if self.Time4D else item_list[0]
